 ...
 * confweb.cpp
 * 
 * library that allows configuration in NVS to be updated
...
#ifdef ESP32
uint8_t ConfWeb::_lastconxresulttmp = WL_IDLE_STATUS;
#endif
...
ConfWebParameter::ConfWebParameter() {
  ConfWebParameter("");
}

ConfWebParameter::ConfWebParameter(const char *custom) {
  _id             = NULL;
  _label          = NULL;
  _length         = 0;
  _value          = nullptr;
...
ConfWebParameter::ConfWebParameter(const char *id, const char *label) {
  init(id, label, "", 0, "", WFM_LABEL_DEFAULT);
}

ConfWebParameter::ConfWebParameter(const char *id, const char *label, const char *defaultValue, int length) {
  init(id, label, defaultValue, length, "", WFM_LABEL_DEFAULT);
}

ConfWebParameter::ConfWebParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom) {
  init(id, label, defaultValue, length, custom, WFM_LABEL_DEFAULT);
}

ConfWebParameter::ConfWebParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  init(id, label, defaultValue, length, custom, labelPlacement);
}

void ConfWebParameter::init(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  _id             = id;
  _label          = label;
...
ConfWebParameter::~ConfWebParameter() {
  ...
  _length=0; // setting length 0, ideally the entire parameter should be removed, or added to ConfWeb scope so it follows
}

ConfWebParameter& ConfWebParameter::operator=(const ConfWebParameter& rhs){
...
void ConfWebParameter::setValue(const char *defaultValue, int length) {
  if(!_id){
...
const char* ConfWebParameter::getValue() const {
  // Serial.println(printf("Address of _value is %p\n", (void *)_value)); 
  return _value;
}
const char* ConfWebParameter::getID() const {
  return _id;
}
const char* ConfWebParameter::getPlaceholder() const {
  return _label;
}
const char* ConfWebParameter::getLabel() const {
  return _label;
}
int ConfWebParameter::getValueLength() const {
  return _length;
}
int ConfWebParameter::getLabelPlacement() const {
  return _labelPlacement;
}
const char* ConfWebParameter::getCustomHTML() const {
  return _customHTML;
}
 ...
 * @access public
 * @param {[type]} ConfWebParameter *p [description]
 */
bool ConfWeb::addParameter(ConfWebParameter *p) {
  ...
  if(_params == NULL){
    #ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_DEV,F("allocating params bytes:"),_max_params * sizeof(ConfWebParameter*));        
    #endif
    _params = (ConfWebParameter**)malloc(_max_params * sizeof(ConfWebParameter*));
  }
  ...
  if(_paramsCount == _max_params){
    ...
    DEBUG_WM(DEBUG_DEV,F("re-allocating params bytes:"),_max_params * sizeof(ConfWebParameter*));    
    #endif
    ConfWebParameter** new_params = (ConfWebParameter**)realloc(_params, _max_params * sizeof(ConfWebParameter*));
    #ifdef WM_DEBUG_LEVEL
 ...
 * @access public
 */
ConfWebParameter** ConfWeb::getParameters() {
  return _params;
}
 ...
 * @access public
 */
int ConfWeb::getParametersCount() {
  return _paramsCount;
}
...
// constructors
ConfWeb::ConfWeb(Print& consolePort):_debugPort(consolePort){
  ConfWebInit();
}
...
ConfWeb::~ConfWeb() {
  _end();
  // parameters
  // @todo below belongs to ConfWebparameter
  if (_params != NULL){
    #ifdef WM_DEBUG_LEVEL
  ...
  // remove event
  // WiFi.onEvent(std::bind(&ConfWeb::WiFiEvent,this,_1,_2));
  #ifdef ESP32
 ...
 */
boolean ConfWeb::autoConnect(char const *apName, char const *apPassword) {
  #ifdef WM_DEBUG_LEVEL
...
bool ConfWeb::setupHostname(bool restart){
  if(_hostname == "") {
    #ifdef WM_DEBUG_LEVEL
...
boolean ConfWeb::configPortalHasTimeout(){
...
void ConfWeb::setupHTTPServer(){
  ...
  server->on(R_root,       std::bind(&ConfWeb::handleRoot, this));
  server->on((R_wifi),       std::bind(&ConfWeb::handleWifi, this, true));
  server->on((R_wifinoscan), std::bind(&ConfWeb::handleWifi, this, false));
  server->on((R_wifisave),   std::bind(&ConfWeb::handleWifiSave, this));
  server->on((R_info),       std::bind(&ConfWeb::handleInfo, this));
  server->on((R_param),      std::bind(&ConfWeb::handleParam, this));
  server->on((R_paramsave),  std::bind(&ConfWeb::handleParamSave, this));
  server->on((R_restart),    std::bind(&ConfWeb::handleReset, this));
  server->on((R_exit),       std::bind(&ConfWeb::handleExit, this));
  server->on((R_close),      std::bind(&ConfWeb::handleClose, this));
  server->on((R_erase),      std::bind(&ConfWeb::handleErase, this, false));
  server->on((R_status),     std::bind(&ConfWeb::handleWiFiStatus, this));
  server->onNotFound (std::bind(&ConfWeb::handleNotFound, this));
  
  server->on((R_update), std::bind(&ConfWeb::handleUpdate, this));
  server->on((R_updatedone), HTTP_POST, std::bind(&ConfWeb::handleUpdateDone, this), std::bind(&ConfWeb::handleUpdating, this));
 ...
 * @return {[type]}      [description]
 */
boolean  ConfWeb::startConfigPortal(char const *apName, char const *apPassword) {
  _begin();
 ...
 * @return {[type]} [description]
 */
uint8_t ConfWeb::processConfigPortal(){
    if(configPortalActive){
      //DNS handler
 ...
 */
bool ConfWeb::shutdownConfigPortal(){
  #ifdef WM_DEBUG_LEVEL
...
uint8_t ConfWeb::connectWifi(String ssid, String pass, bool connect) {
  #ifdef WM_DEBUG_LEVEL
 ...
 */
bool ConfWeb::wifiConnectNew(String ssid, String pass,bool connect){
  bool ret = false;
  #ifdef WM_DEBUG_LEVEL
 ...
 * @return bool success
 */
bool ConfWeb::wifiConnectDefault(){
  bool ret = false;
...
void ConfWeb::updateConxResult(uint8_t status){
...
uint8_t ConfWeb::waitForConnectResult() {
  #ifdef WM_DEBUG_LEVEL
 ...
 */
uint8_t ConfWeb::waitForConnectResult(uint32_t timeout) {
  if (timeout == 0){
    #ifdef WM_DEBUG_LEVEL
...
String ConfWeb::getHTTPHead(String title){
  String page;
...
void ConfWeb::HTTPSend(const String &content){
 ...
 */
void ConfWeb::handleWifi(boolean scan) {
  #ifdef WM_DEBUG_LEVEL
...
// bool ConfWeb::WiFi_scanNetworksForAP(bool force){
//   WiFi_scanNetworks(force);
// }

void ConfWeb::WiFi_scanComplete(int networksFound){
  _lastscan = millis();
...
bool ConfWeb::WiFi_scanNetworks(){
  return WiFi_scanNetworks(false,false);
}
 
bool ConfWeb::WiFi_scanNetworks(unsigned int cachetime,bool async){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,async);
}
bool ConfWeb::WiFi_scanNetworks(unsigned int cachetime){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,false);
}
bool ConfWeb::WiFi_scanNetworks(bool force,bool async){
    #ifdef WM_DEBUG_LEVEL
    ...
    if(force){
      ...
      if(async && _asyncScan){
          ...
          WiFi.scanNetworksAsync(std::bind(&ConfWeb::WiFi_scanComplete,this,_1));
          #else
...
String ConfWeb::ConfWeb::getScanItemOut(){
    String page;
...
String ConfWeb::getIpForm(String id, String title, String value){
  ...
  if(_paramsCount > 0){
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      if (_params[i] == NULL || _params[i]->_length > 99999) {
        ...
        #ifdef WM_DEBUG_LEVEL
        DEBUG_WM(DEBUG_ERROR,F("[ERROR] ConfWebParameter is out of scope"));
        #endif
        return "";
      }
    }
...
void ConfWeb::handleParamSave() {
  ...
  doParamSave();
  server->sendHeader("Location", "/info",true);  
...
String ConfWeb::getInfoData(String id){
...
// void ConfWeb::handleErase() {
//   handleErase(false);
// }
void ConfWeb::handleErase(boolean opt) {
  #ifdef WM_DEBUG_LEVEL
...
void ConfWeb::stopCaptivePortal(){
...
void ConfWeb::reportStatus(String &page){
 ...
 */
void ConfWeb::setTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void ConfWeb::setConfigPortalTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void ConfWeb::setConnectTimeout(unsigned long seconds) {
 ...
 */
void ConfWeb::setConnectRetries(uint8_t numRetries){
 ...
 */
void ConfWeb::setCleanConnect(bool enable){
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void ConfWeb::setSaveConnectTimeout(unsigned long seconds) {
 ...
 */
void ConfWeb::setSaveConnect(bool connect) {
 ...
 */
void ConfWeb::setDebugOutput(boolean debug) {
  _debug = debug;
  if(_debug && _debugLevel == DEBUG_DEV) debugPlatformInfo();
}

void ConfWeb::setDebugOutput(boolean debug, String prefix) {
  _debugPrefix = prefix;
 ...
 */
void ConfWeb::setAPStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _ap_static_ip = ip;
  _ap_static_gw = gw;
  _ap_static_sn = sn;
}
 ...
 * @param {[type]} IPAddress sn [description]
 */
void ConfWeb::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _sta_static_ip = ip;
  _sta_static_gw = gw;
  _sta_static_sn = sn;
}
 ...
 */
void ConfWeb::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn, IPAddress dns) {
 ...
 */
void ConfWeb::setMinimumSignalQuality(int quality) {
 ...
 */
void ConfWeb::setBreakAfterConfig(boolean shouldBreak) {
 ...
 * @access public 
 * @param {[type]} void (*func)(ConfWeb* wminstance)
 */
void ConfWeb::setAPCallback( std::function<void(ConfWeb*)> func ) {
  _apcallback = func;
}
 ...
 */
void ConfWeb::setWebServerCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ConfWeb::setSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ConfWeb::setPreSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void(*func)(void)
 */
void ConfWeb::setConfigResetCallback( std::function<void()> func ) {
    _resetcallback = func;
}
 ...
 * @param {[type]} void (*func)(void)
 */
void ConfWeb::setSaveParamsCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ConfWeb::setPreSaveParamsCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ConfWeb::setPreOtaUpdateCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void ConfWeb::setConfigPortalTimeoutCallback( std::function<void()> func ) {
 ...
 * @param char element
 */
void ConfWeb::setCustomHeadElement(const char* html) {
 ...
 * @param char element
 */
void ConfWeb::setCustomMenuHTML(const char* html) {
 ...
 */
void ConfWeb::setRemoveDuplicateAPs(boolean removeDuplicates) {
 ...
 */
void ConfWeb::setConfigPortalBlocking(boolean shouldBlock) {
 ...
 */
void ConfWeb::setRestorePersistent(boolean persistent) {
 ...
 */
void ConfWeb::setShowStaticFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void ConfWeb::setShowDnsFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void ConfWeb::setShowPassword(boolean show){
  _showPassword = show;
}
 ...
 */
void ConfWeb::setCaptivePortalEnable(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void ConfWeb::setWiFiAutoReconnect(boolean enabled){
 ...
 */
void ConfWeb::setAPClientCheck(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void ConfWeb::setWebPortalClientCheck(boolean enabled){
 ...
 * @param boolean enabled [false]
 */
void ConfWeb::setScanDispPerc(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void ConfWeb::setEnableConfigPortal(boolean enable)
{
 ...
 * @param boolean enabled [true]
 */
void ConfWeb::setDisableConfigPortal(boolean enable)
{
 ...
 */
bool  ConfWeb::setHostname(const char * hostname){
  //@todo max length 32
  _hostname = String(hostname);
  return true;
}

bool  ConfWeb::setHostname(String hostname){
  //@todo max length 32
  _hostname = hostname;
  return true;
}
 ...
 */
void ConfWeb::setWiFiAPChannel(int32_t channel){
  _apChannel = channel;
}
 ...
 */
void ConfWeb::setWiFiAPHidden(bool hidden){
  _apHidden = hidden;
}
 ...
 */
void ConfWeb::setShowInfoErase(boolean enabled){
 ...
 * @param boolean enabled
 */
void ConfWeb::setShowInfoUpdate(boolean enabled){
 ...
 */
bool ConfWeb::getConfigPortalActive(){
 ...
 * @return bool true if active
 */
bool ConfWeb::getWebPortalActive(){
  return webPortalActive;
}


String ConfWeb::getWiFiHostname(){
  #ifdef ESP32
 ...
 */
void ConfWeb::setTitle(String title){
  _title = title;
}
 ...
 *  ConfWeb.setMenu(menu);
 * @since $dev
 * @param uint8_t menu[] array of menu ids
 */
void ConfWeb::setMenu(const char * menu[], uint8_t size){
#ifdef WM_DEBUG_LEVEL
 ...
 */
void ConfWeb::setMenu(std::vector<const char *>& menu){
#ifdef WM_DEBUG_LEVEL
 ...
 * @since $dev
 */
void ConfWeb::setParamsPage(bool enable){
 ...
 */
String ConfWeb::getConfigPortalSSID() {
  return _apName;
}
 ...
 */
uint8_t ConfWeb::getLastConxResult(){
 ...
 * @return string 
 */
String ConfWeb::getDefaultAPName(){
 ...
 */
void ConfWeb::setCountry(String cc){
  _wificountry = cc;
}
 ...
 */
void ConfWeb::setClass(String str){
  _bodyClass = str;
}
 ...
 */
void ConfWeb::setDarkMode(bool enable){
 ...
 */
void ConfWeb::setHttpPort(uint16_t port){
  _httpPort = port;
}


bool ConfWeb::preloadWiFi(String ssid, String pass){
  _defaultssid = ssid;
  _defaultpass = pass;
  return true;
}
 ...
 * @return String
 */
String ConfWeb::getWiFiSSID(bool persistent){
 ...
 * @return String
 */
String ConfWeb::getWiFiPass(bool persistent){
...
void ConfWeb::DEBUG_WM(Generic text) {
  DEBUG_WM(DEBUG_NOTIFY,text,"");
}

template <typename Generic>
void ConfWeb::DEBUG_WM(wm_debuglevel_t level,Generic text) {
  if(_debugLevel >= level) DEBUG_WM(level,text,"");
}

template <typename Generic, typename Genericb>
void ConfWeb::DEBUG_WM(Generic text,Genericb textb) {
  DEBUG_WM(DEBUG_NOTIFY,text,textb);
}

template <typename Generic, typename Genericb>
void ConfWeb::DEBUG_WM(wm_debuglevel_t level,Generic text,Genericb textb) {
 ...
 * @return {[type]} [description]
 */
void ConfWeb::debugSoftAPConfig(){
 ...
 * @return {[type]} [description]
 */
void ConfWeb::debugPlatformInfo(){
  #ifdef ESP8266
...
int ConfWeb::getRSSIasQuality(int RSSI) {
  int quality = 0;
...
/** Is this an IP? */
boolean ConfWeb::isIp(String str) {
...
/** IP to String? */
String ConfWeb::toStringIp(IPAddress ip) {
  String res = "";
  for (int i = 0; i < 3; i++) {
...
boolean ConfWeb::validApPassword(){
 ...
 */
String ConfWeb::htmlEntities(String str, bool whitespace) {
  str.replace("&","&amp;");
  str.replace("<","&lt;");
  str.replace(">","&gt;");
  str.replace("'","&#39;");
 ...
 * @return {[type]}         [description]
 */
String ConfWeb::getWLStatusString(uint8_t status){
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String ConfWeb::getWLStatusString(){
  uint8_t status = WiFi.status();
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String ConfWeb::encryptionTypeStr(uint8_t authmode) {
#ifdef WM_DEBUG_LEVEL
...
String ConfWeb::getModeString(uint8_t mode){
  if(mode <= 3) return WIFI_MODES[mode];
  return FPSTR(S_NA);
}

bool ConfWeb::WiFiSetCountry(){
...
bool ConfWeb::WiFi_Mode(WiFiMode_t m,bool persistent) {
    bool ret;
    #ifdef ESP8266
...
bool ConfWeb::WiFi_Mode(WiFiMode_t m) {
	return WiFi_Mode(m,false);
}

// sta disconnect without persistent
bool ConfWeb::WiFi_Disconnect() {
    #ifdef ESP8266
...
bool ConfWeb::WiFi_enableSTA(bool enable,bool persistent) {
#ifdef WM_DEBUG_LEVEL
...
bool ConfWeb::WiFi_enableSTA(bool enable) {
	return WiFi_enableSTA(enable,false);
}

bool ConfWeb::WiFi_eraseConfig() {
    #ifdef WM_DEBUG_LEVEL
...
uint8_t ConfWeb::WiFi_softap_num_stations(){
  #ifdef ESP8266
...
bool ConfWeb::WiFi_hasAutoConnect(){
  return WiFi_SSID(true) != "";
}

String ConfWeb::WiFi_SSID(bool persistent) const{
...
String ConfWeb::WiFi_psk(bool persistent) const {
    #ifdef ESP8266
    struct station_config conf;
  ...
  void ConfWeb::WiFiEvent(WiFiEvent_t event,arduino_event_info_t info){
  #else
  void ConfWeb::WiFiEvent(WiFiEvent_t event,system_event_info_t info){
...
void ConfWeb::WiFi_autoReconnect(){
  #ifdef ESP8266
    ...
    // if(_wifiAutoReconnect){
      ...
      if(wm_event_id == 0) wm_event_id = WiFi.onEvent(std::bind(&ConfWeb::WiFiEvent,this,_1,_2));
    // }
  #endif
}

// Called when /update is requested
void ConfWeb::handleUpdate() {
  #ifdef WM_DEBUG_LEVEL
