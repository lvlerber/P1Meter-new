 ...
 * ConfWeb.h
 * 
 * ConfWeb, a library for the ESP8266/Arduino platform
...
// #pragma message "VER_ARDUINO_STR = " WM_STRING(VER_ARDUINO_STR)
    ...
    #define WIFI_MANAGER_MAX_PARAMS 500 // params will autoincrement and realloc by this amount when max is reached
#endif
...
class ConfWebParameter {
  public:
    /** 
        Create custom parameters that can be added to the ConfWeb setup web page
        @id is used for HTTP queries and must not contain spaces nor other special characters
    */
    ConfWebParameter();
    ConfWebParameter(const char *custom);
    ConfWebParameter(const char *id, const char *label);
    ConfWebParameter(const char *id, const char *label, const char *defaultValue, int length);
    ConfWebParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom);
    ConfWebParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement);
    ~ConfWebParameter();
    ConfWebParameter& operator=(const ConfWebParameter& rhs);
  ...
  private:
   // ConfWebParameter& operator=(const ConfWebParameter&);
    const char *_id;
    const char *_label;
    char       *_value;
    int         _length;
...
class ConfWeb
{
  public:
    ...
    bool          addParameter(ConfWebParameter *p);

    //returns the list of Parameters
    ConfWebParameter** getParameters();
    ...
    void          setAPCallback( std::function<void(ConfWeb*)> func );
    ...
    // set the webapp title, default ConfWeb
  ...
  private:
    ...
    String        _wifissidprefix         = S_ssidpre; // auto apname prefix prefix+chipid
    ...
    bool          _aggresiveReconn        = true; // use an agrressive reconnect strategy, WILL delay conxs
                                                   ...
                                                   // https://github.com/tzapu/ConfWeb/issues/1067
    ...
    String        _title                  = FPSTR(S_brand); // app title -  default ConfWeb
    ...
    std::function<void(ConfWeb*)> _apcallback;
